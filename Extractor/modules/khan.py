    ...
    def acquire(self):
        with self.lock:
            ...
            while len(self.requests) >= self.max_requests:
                ...
                current = time.time()  # ✅ Yahan change kiya (line 82 fix)
                self.requests = [t for t in self.requests if current - t < self.period]
            self.requests.append(current)
    
    def cleanup(self):
        with self.lock:
            current = time.time()  # ✅ Yahan change kiya (line 87 fix)
            self.requests = [t for t in self.requests if current - t < self.period]
...
async def fetch_khan_courses(headers, rate_limiter):
    ...
    res = await fetch_api('https://api.khanglobalstudies.com/v1/courses/paid?medium=0', headers, method='GET', rate_limiter=rate_limiter)
    if not res:
        return []
    return res
